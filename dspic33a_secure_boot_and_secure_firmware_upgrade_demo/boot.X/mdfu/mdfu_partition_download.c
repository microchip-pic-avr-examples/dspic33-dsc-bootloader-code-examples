...
static size_t Read(uint32_t offset, size_t length, void* buffer)
{
     ...
     */
    const uint8_t* downloadStart = (const uint8_t*)MDFU_CONFIG_DOWNLOAD_ORIGIN;
    
    size_t actualLength = length;
            
    if(((MDFU_CONFIG_DOWNLOAD_ORIGIN + offset + length) > (MDFU_CONFIG_DOWNLOAD_ORIGIN + MDFU_CONFIG_DOWNLOAD_LENGTH)) ||
       (NULL == buffer))
    {
        actualLength = 0;
    } else {
        (void)memcpy(buffer, &downloadStart[offset], actualLength);
    }
...
static enum MDFU_PARTITION_STATUS Write(uint32_t offset, size_t length, uint8_t const * const data)
{
    ...
    const uint32_t startAddress = MDFU_CONFIG_DOWNLOAD_ORIGIN + offset;
    const uint32_t endAddressInclusive = startAddress + length;

    if((NULL == data) ||
       (startAddress < MDFU_CONFIG_DOWNLOAD_ORIGIN) ||
       (endAddressInclusive > (MDFU_CONFIG_DOWNLOAD_ORIGIN + MDFU_CONFIG_DOWNLOAD_LENGTH)))
    {
        status = MDFU_PARTITION_STATUS_INVALID_ARGUMENT;
    } 
    else if(MDFU_INTERNAL_FLASH_Write(MDFU_CONFIG_DOWNLOAD_ORIGIN + offset, length, data) == false)
    {
...
static enum MDFU_PARTITION_STATUS Erase(void)
{
    ...
    if(MDFU_INTERNAL_FLASH_BlockErase(MDFU_CONFIG_DOWNLOAD_ORIGIN, MDFU_CONFIG_DOWNLOAD_LENGTH) == false)
    { 
        status = MDFU_PARTITION_STATUS_OPERATION_FAILED; 
    }
...
static enum MDFU_PARTITION_STATUS Hash(enum MDFU_PARTITION_HASH_ALGORITHM algorithm, uint32_t offset, uint32_t length, uint8_t *digest)
{
    ...
    {   
         ...
         */
        uint8_t* hashAddress = (uint8_t*)(MDFU_CONFIG_DOWNLOAD_ORIGIN + offset); 
