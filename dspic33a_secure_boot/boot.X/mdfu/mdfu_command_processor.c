...
#if ((MDFU_CONFIG_EXECUTABLE_HEADER_ORIGIN + MDFU_CONFIG_EXECUTABLE_HEADER_LENGTH) != MDFU_CONFIG_EXECUTABLE_DATA_ORIGIN )
...
#if ((MDFU_CONFIG_DOWNLOAD_HEADER_ORIGIN + MDFU_CONFIG_DOWNLOAD_HEADER_LENGTH) != MDFU_CONFIG_DOWNLOAD_DATA_ORIGIN )
...
static void handleTransportComplete(enum MDFU_SEQUENCE_STATE sequenceState, command_parser* command, response_parser* response, response_parser* errorResponse, enum MDFU_COMMAND_RESULT* sessionState);
static void mdfuGetErrorResponse(response_parser* response);
static void mdfuGetErrorResponseWithData(response_parser* response, uint8_t data);
static void sendErrorResponseWithData(response_parser* response, uint8_t data);
...
enum MDFU_COMMAND_RESULT MDFU_CommandProcess(void)
{
    ...
    switch(receiveCommand(&currentCommand))
    {
        case MDFU_TRANSPORT_COMPLETE:
            handleTransportComplete(checkSequenceNumber(currentCommand.c_sequence), &currentCommand, &lastResponse, &errorResponse, &sessionState);
            break;
        case MDFU_TRANSPORT_FAIL_INTEGRITY_CHECK_ERROR:
            sendErrorResponseWithData(&errorResponse, MDFU_TRANSPORT_INTEGRITY_CHECK_ERROR);
            break;
        case MDFU_TRANSPORT_FAIL_TOO_LONG:
            sendErrorResponseWithData(&errorResponse, MDFU_COMMAND_TOO_LONG);
            break;
        case MDFU_TRANSPORT_FAIL_TOO_SHORT:
            sendErrorResponseWithData(&errorResponse, MDFU_COMMAND_TOO_SHORT);
            break;
...
static enum MDFU_COMMAND_RESULT executeCommand (command_parser* command, response_parser* response)
{
    ...
    switch(command->code)
    {
        ...
        case MDFU_COMMAND_START_TRANSFER: 
            MDFU_FirmwareUpdateInitialize();
            
            //At this point always assume transfer is ok
            if(MDFU_FirmwareUpdateEraseMemory(&download) == MDFU_FIRMWARE_UPDATE_PROTOCOL_PASS )
            {
                result = MDFU_COMMAND_SESSION_IN_PROGRESS;
            }             
            else
        ...
        case MDFU_COMMAND_WRITE_CHUNK:
            if(MDFU_FirmwareUpdateWriteMemory(&download, command->dataPayload, command->dataLength) == MDFU_FIRMWARE_UPDATE_PROTOCOL_PASS)
            {
                result = MDFU_COMMAND_SESSION_IN_PROGRESS;
            } 
            else
        ...
        case MDFU_COMMAND_GET_IMAGE_STATE:
            if(MDFU_Verify(&download) != MDFU_VERIFY_CODE_SUCCESS)
            {
...
static void handleTransportComplete(enum MDFU_SEQUENCE_STATE sequenceState, command_parser* command, response_parser* response, response_parser* errorResponse, enum MDFU_COMMAND_RESULT* sessionState)
{
    switch(sequenceState)
    {
        case MDFU_SEQUENCE_STATUS_CONTINUE_PROCESSING:
            *sessionState = executeCommand(command, response);
        ...
        default:
            ...
            sendErrorResponseWithData(errorResponse, MDFU_SEQUENCE_NUMBER_INVALID);
            break;
    }
}
static void mdfuGetErrorResponse(response_parser* response)
...
static void sendErrorResponseWithData(response_parser* response, uint8_t data)
{
    mdfuGetErrorResponseWithData(response, data);
    sendResponse(response);
}
