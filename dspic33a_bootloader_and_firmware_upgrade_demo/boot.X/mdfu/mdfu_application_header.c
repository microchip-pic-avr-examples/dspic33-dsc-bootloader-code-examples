 ...
 * @param    const enum MDFU_APPLICATION_HEADER_CODE type - the type of the last entry read
 * @return   bool - true if we have reached the end of the header, false otherwise
 */
static bool IsEndOfHeader(const uint32_t offset, const enum MDFU_APPLICATION_HEADER_CODE type)
{
    bool endOfHeader = false;
    
    if((MDFU_APPLICATION_HEADER_CODE_END_OF_HEADER == type) ||
...
/**
 * @brief    Scans application header for requested entry
 * @param    struct IMAGE* image - pointer to the image to search
 * @param    enum MDFU_APPLICATION_HEADER_CODE code - the type code of item to find
 ...
 */
size_t MDFU_AppHeaderItemBufferRead(const struct PARTITION *partition, const enum MDFU_APPLICATION_HEADER_CODE code, const size_t requestedLength, void* buffer)
{
    enum MDFU_APPLICATION_HEADER_CODE type;
    uint32_t length = 0;
    size_t dataRead = 0;
       ...
       (NULL != buffer))
    {
        ...
        do
        {
            partition->read(entryOffset + TYPE_OFFSET, sizeof(enum MDFU_APPLICATION_HEADER_CODE), &type);
