...
{
    .receiveIndex = 0,
    .bufferPtr = (uint8_t *) & receiveBuffer,
...
static void HandleEndOfFrame(void) 
{
    ...
    if((calculatedFrameCheck != receivedFrameCheck) ||
       (commandReceiver.receiveIndex < MINIMUM_DECODED_FRAME_SIZE))
    {
        commandReceiver.status = MDFU_TRANSPORT_FAIL;
    }
    else
